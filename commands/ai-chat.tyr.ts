...
import { TASK_PRIORITIES } from '@tyrframework/cli';
 ...
 * @example
 * tyr ai:chat
 * // Chats about the current directory
 *
 * @example
 * tyr ai:chat ./src
 * // Chats about ./src (relative or absolute)
 *
 * @example
 * tyr ai:chat ./src --port 4000 --split 0.6 --priority alta-prioridad
 * // Custom port, chat/file-browser split ratio, and routing priority
 */
export default ({ fail, logger, fs, path, aiContext, aiVendor, chat, prompts, tokens, memory, git }: TyrContext) => {
    return async (args: string[]) => {
        ...
        const targetDir = positional[0] ? path.resolve(cwd, positional[0]) : cwd;
        ...
        const fileIndex = buildFileIndex(await indexFiles(fs, path, targetDir));
        ...
        async function readContextFileContent(relPath: string): Promise<string> {
            ...
            const content = await fs.read(path.join(targetDir, relPath));
        ...
        async function generateReply(fs: any, messages: any[], sessionId: string): Promise<{ text: string; diffs: any[] }> {
            const diffs: any[] = [];
        ...
        chat.onMessage(async ({ message, history, dir, sessionId }: any) => {
            ...
            const reply = await generateReply(fs, messages, sessionId);
